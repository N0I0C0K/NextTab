...
import { useStorage, PERMISSION_ORIGINS, usePermission } from '@/utils'
import {
  addWallpaperHistory,
...
import type { WallhavenSortMode } from '@/utils/storage'
import {
  Button,
  Stack,
  Text,
...
import { type FC, type UIEvent, useCallback, useEffect, useState, useRef } from 'react'
...
export const WallpaperSettings: FC = () => {
  ...
  // Use a ref to prevent duplicate requests before React applies the loading state.
  ...
  const handleScroll = useCallback(
    (event: UIEvent<HTMLDivElement>) => {
      ...
      if (isNearScrollBottom({ scrollTop, scrollHeight, clientHeight }, SCROLL_THRESHOLD)) {
        void fetchWallpapers(currentPageRef.current + 1, true)
      }
    },
    [fetchWallpapers],
  )
  ...
  const handleSortModeChange = useCallback(
    async (mode: WallhavenSortMode) => {
      await updateSettings({ wallhavenSortMode: mode })
      setCurrentPage(1)
      setHasMore(true)
      setWallpapers([])
  ...
  const handleSelectWallpaper = useCallback(async (url: string, thumbnailUrl: string) => {
    ...
    await updateSettings({ wallpaperUrl: url, wallpaperType: 'url' })
    await addWallpaperHistory(url, thumbnailUrl)
  }, [])
  ...
  const handleDeleteHistoryItem = useCallback(async (url: string) => {
    await removeWallpaperHistory(url)
  }, [])
      ...
      <Input
        ...
        onChange={e => {
          ...
          if (newUrl && settings.wallpaperType === 'local') {
            updateSettings({ wallpaperUrl: newUrl, wallpaperType: 'url' })
          } else {
            updateSettings({ wallpaperUrl: newUrl })
          }
        }}
      />
      ...
      {wallhavenPermission.isGranted && (
        <>
          ...
          {isLoading && wallpapers.length === 0 ? (
            ...
            <div data-testid="wallpaper-gallery" className="h-[300px] overflow-y-auto pr-2" onScroll={handleScroll}>
