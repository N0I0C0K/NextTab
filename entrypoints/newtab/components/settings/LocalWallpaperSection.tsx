...
import { localWallpaperStorage, setLocalWallpaper, settingStorage, updateSettings } from '@/utils/storage'
...
export const LocalWallpaperSection: FC = () => {
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await setLocalWallpaper(imageData)
      await updateSettings({ wallpaperType: 'local' })
    } catch (err) {
  ...
  const handleClear = useCallback(async () => {
    await setLocalWallpaper(null)
    await updateSettings({ wallpaperType: 'url' })
  }, [])
  ...
  return (
    <>
      ...
      <input
        data-testid="local-wallpaper-input"
        ref={fileInputRef}
        type="file"
